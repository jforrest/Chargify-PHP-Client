...
class ChargifySubscription
{
	...
	private $connector;
	public function __construct(ChargifyConnector $connector, SimpleXMLElement $subscription_xml_node = null)
	{
		...
		if ($subscription_xml_node) {
			...
			foreach($subscription_xml_node as $key => $element)
			{
			  	if($key == 'customer' || $key == 'customer_attributes') {
		  			$this->customer = new ChargifyCustomer($this->connector, $element);
			  	} else if($key == 'product') {
			  		$this->product = new ChargifyProduct($this->connector, $element);
			  		...
			  		foreach ($element as $component) {
			  			$this->components[] = new ChargifyQuantityBasedComponent($this->connector, $component);
			  		}
			  	} else {
